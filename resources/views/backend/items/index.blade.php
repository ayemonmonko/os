	...
	<table class="table table-bordered">
		...
		<tbody>
			...
			<td>{{$item->codeno}} <a href="{{route('items.show',$item->id)}}"><span class="badge badge-primary badge-pill">Detail</span></a>
				<a href="#"><span class="box badge badge-primary badge-pill" data-name="{{$item->name}}" data-photo="{{asset($item->photo)}}" data-price="{{$item->price}}" data-desc="{{$item->description}}"  data-toggle="modal" data-target="#itemModal">Modal</span></a>
			</td>
			<td>{{$item->name}}</td>
			<td>{{$item->price}}MMK</td>
			<td><a href="{{route('items.edit',$item->id)}}" class="btn btn-warning">Edit</a>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="row">
        	...
        	<div class="col-md-6">
        		Price:<strong id="price">MMK</strong><br>
